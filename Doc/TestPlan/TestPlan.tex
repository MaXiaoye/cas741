...
\wss{You have a test where $E$ is not a number, but I don't see a test where $E$
  is a number, but not in the correct range.  Okay, I found the test later in
  the document.  These tests are so simple and repetitive, it would be better to
  summarize them in a table, where each row corresponds to a different test.}
...
Output: initial speed $v_{0} = 10 * E$  \wss{Why?  How was this derived?  The
  reader should be able to verify the test cases and there isn't enough
  information here to do the verification.}
...
How test will be performed: Manually input and check output by developer. \an{Here we take $(X,Y,Z) = (0,0,0)$ as input case for System test. Test cases for different values will be covered in unit test part section \ref{Sec_UnitTest}}
...
\wss{Rather than be general, you should give specific input values and specific
  ...
  Again, it isn't clear why this has to be a manual test.}
...
How test will be performed: Manually input and check output by developer. \an{Here we take $E = 5$ and  $(X,Y,Z) = (0,0,0)$ as input case for System test. Test cases for different values will be covered in unit test part section \ref{Sec_UnitTest}}
...
\wss{How will the comparison be assessed? quantified?}
...
\wss{It is unclear how this test differs from the previous centre of gravity
  test?}
...
\begin{enumerate}
	...
	How test will be performed: Automated unit testing.\an{Will perform finite test cases for different input in valid range.}
	...
	Output: $a = \mu_{k}g$, $S_{x}=v_{0}\cdot cos\theta _{1}\cdot cos\theta _{2}\cdot t-\frac{1}{2}at^{2}$ and $S_{y}=v_{0}\cdot cos\theta _{1}\cdot sin\theta _{2}\cdot t-\frac{1}{2}at^{2}$ \an{Acceleration $a$ is an intermediate result that doesn't need separate module and system test.}
