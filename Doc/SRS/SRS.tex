...
not n).} \an{Fixed by explaining $n$ and format it as a mathematical variable.}
\\
...
Breaking effect presents how the pieces of an object move after it separates into parts with suddenness or violence. \an{Fixed by explaining BE here} 
...
The organization of this document follows the template for an SRS for scientific computing software proposed by \citet{SmithAndLai2005, SmithEtAl2007}. \an{Copy the format from original template.} The
...
The physical system of \progname{} includes visual terrain \wss{spell check!} and a target 3-D object which will break into pieces. \an{Fixed. Sorry for the mistake !}
...
\begin{minipage}{\textwidth}
	...
	\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
		...
		SI Units & N\\
		\hline
		Equation&$F_{k}=\mu_{k}F_{n}=\mu_{k}mg$,  $F_{k}=ma$\\
		\hline
		Description & 
...
\wss{You don't say how the object will be broken into pieces.  You have this
  ...
  to your traceability matrix, so that you would see this omission.} \an{I prefer to use existing function provided by Unity3D because how to split the object into pieces is not emphasis. I can specify this in system constrain and delete object cutting in GS1.}
