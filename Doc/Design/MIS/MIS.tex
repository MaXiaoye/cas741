...
\noindent DisAirCalX():\\
Equation: $v_{0}=10*E ,S_{x}=v_{0}\cdot cos\theta _{1}\cdot cos\theta _{2}\cdot t$ \an{Based on assumption8 in SRS that value of initial velocity given by explosion is ten times input $E$ unit length in unity per second.}
\begin{itemize}
	\item transition: None
